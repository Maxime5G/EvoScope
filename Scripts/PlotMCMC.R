...
histogram_parameter_MCMC <- function(x, prefix){
    ...
    for (i in 3:(ncol(subtable)-1)){
        ...
        hist(subtable[,i], breaks=100, freq=F, main=maintitle, xlab=colnames(subtable)[i], col='#56B4E9')
        lines(density(subtable[,i]), col='#D55E00', lwd=3)
        box()
    }
    hist(subtable[,'log.posterior'], breaks=100, freq=F, main='histogram of the log-posterior', xlab='log posterior', col='#56B4E9')
    lines(density(subtable[,'log.posterior']), col='#D55E00', lwd=3)
    dev.off()
}

if(nchar(myt$tvector[1])>1){barplot_tvectors_MCMC(myt$tvector, thePrefix)}
...
out.df <- c(apply(myt[,seq(2,10)], MARGIN=2, FUN=estimate_mode))
print(out.df)